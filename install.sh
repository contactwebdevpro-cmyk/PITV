#!/bin/bash
# ╔══════════════════════════════════════════════════════════════╗
# ║          PiTV OS - Script d'Installation v1.0               ║
# ║       Compatible: Raspberry Pi 3B+ (OS & OS Lite)           ║
# ╚══════════════════════════════════════════════════════════════╝
...
detect_pi() {
  ...
  if grep -q "Raspberry Pi" /proc/cpuinfo 2>/dev/null; then
    ...
    warn "Pas de Raspberry Pi détecté — installation en mode compatibilité"
    PI_MODEL="Inconnu"
  fi
...
Type=simple
EOF
  ok "Autologin configuré pour l'utilisateur 'pi'"
...
'
  # Ajoute pour l'utilisateur pi
  if id "pi" &>/dev/null; then
    if ! grep -q "PiTV OS" /home/pi/.bash_profile 2>/dev/null; then
      echo "$BASH_PROFILE_CONTENT" >> /home/pi/.bash_profile
    fi
    ok "Auto-start X configuré pour 'pi'"
  fi
...
*/10 * * * * pi DISPLAY=:0 xset s off -dpms 2>/dev/null || true
CRON
